...
def test(request):
    ...
    time_time_json = []
    while i < 1:
        b, e = get_time_str(i, i + 0.0033333)
        time_time_json.append([b, e])
        i += 0.0033333
    print(len(time_time_json))
    jsonData = json.dumps(time_time_json)
    fileObject = open('time_300_slices.json', 'w')
    fileObject.write(jsonData)
    fileObject.close()

    # with open(str(BASE_DIR) + '//clustering.json', 'r', encoding='utf8')as fp:
    #     json_data = json.load(fp)
    #
    # data = json_data['allData']
    # for i in range(len(data)):  # region
    #     region = data[i]
    #     for j in range(len(region['Region_VPC'])):  # VPC
    #         vpc = region['Region_VPC'][j]
    #         for k in range(len(vpc['AS_ECS'])):  # AS
    #             az = vpc['AS_ECS'][k]
    #             az.pop('VPC_ID')
    #             az.pop('Region_ID')
    #             # data[i]['Region_VPC'][j]['AS_ECS'][k] = az
    #
    # jsonData = json.dumps(data)
    # fileObject = open('cluster.json', 'w')
    # fileObject.write(jsonData)
    # fileObject.close()

    # begin_time = 0
    # end_time = 0.01
    # b, e = get_time_str(begin_time, end_time)
    # print(b, e)
    # print(2 + 8)
    # slice = {
    #     'beginTime': 0,
    ...
    #         'color': data_color_dict[ecs_force_and_file[i]['ESC_ID']]
    #     })
    #
    # AS_ECS = []
    # AS_ECS_set = set()
    ...
    # fileObject = open('clustering.json', 'w')
    # fileObject.write(jsonData)
    # fileObject.close()
